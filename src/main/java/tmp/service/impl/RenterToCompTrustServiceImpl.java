...
public class RenterToCompTrustServiceImpl implements RenterToCompTrustService {
    ...
    public BigDecimal calcRenterToCompTrust(Renter renter, Component component) {
        ...
        //查询租户与组件的交互历史次数
        List<RenterHistory> renterHistories = renterHistoryMapper.selectByTrustorAndTrusteeUid(renterUid,componentUid, null);
        int directTimes = renterHistories.size();
        //查询租户与所有组件的交互历史次数
        List<RenterHistory> histories = renterHistoryMapper.selectByTrustorAndTrusteeUid(renterUid, null, 2);
    ...
    public BigDecimal calcRenterToCompDirectTrust(Renter renter, Component component) {
        ...
        ProviderTrustValue providerTrustValue = providerTrustValueMapper.queryLatestByProviderUid(component.getParentUid());
    ...
    public BigDecimal calcRenterToCompIndirectTrust(Renter renter, Component component) {
        ...
        List<RenterTrustValue> renterTrustValues = renterTrustValueMapper.selectByTrustorAndTrusteeUid(null, componentUid);
        //获得推荐人名单，去重，去自身
        ...
        for (String recommenderUid : recommenders) {
            //查询推荐者对组件的最近信任值
            RenterTrustValue recommenderToRenterTrust = renterTrustValueMapper.queryLatestTrustValue(recommenderUid, componentUid);
            BigDecimal recommenderDirectTrustValue = recommenderToRenterTrust.getTrustValue();
            //查询请求者对推荐者的最近信任值
            RenterTrustValue renterToRecommenderTrust = renterTrustValueMapper.queryLatestTrustValue(renterUid, recommenderUid);
