...
public class CompToRenterTrustServiceImpl implements CompToRenterTrustService {
    ...
    public BigDecimal calcCompToRenterTrust(Component component, Renter renter) {
        ...
        //查询组件与租户的交互历史次数
        List<ComponentHistory> componentHistories = componentHistoryMapper.selectByTrustorAndTrusteeUid(componentUid, renterUid, null);
        int directTimes = componentHistories.size();
        //查询组件与所有租户的交互历史次数
        List<ComponentHistory> histories = componentHistoryMapper.selectByTrustorAndTrusteeUid(componentUid, null, 2);
    ...
    public BigDecimal calcCompToRenterIndirectTrust(Component component, Renter renter) {
        ...
        List<ComponentTrustValue> componentTrustValues = componentTrustValueMapper.selectByTrustorAndTrusteeUid(null, renterUid);
        //获得推荐人名单，去重，去自身
        ...
        for (String recommenderUid : recommenders) {
            //查询推荐者对租户的最近信任值
            ComponentTrustValue recommenderToRenterTrust = componentTrustValueMapper.queryLatestTrustValue(recommenderUid, renterUid);
            BigDecimal recommenderDirectTrustValue = recommenderToRenterTrust.getTrustValue();
            //查询请求者对推荐者的最近信任值
            ComponentTrustValue componentToRecommenderTrust = componentTrustValueMapper.queryLatestTrustValue(componentUid, recommenderUid);
